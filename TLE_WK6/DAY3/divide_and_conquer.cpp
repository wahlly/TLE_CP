...
int main(){
    ...
    while(t--){
        ...
        cin >> n;
        int arr[n];
        int odd = 0;    //counting number of odd elements
        for(int i=0; i<n; i++){
            cin >> arr[i];
            if(arr[i] % 2 != 0){
                odd++;
        ...
        int ans = INT_MAX;
        if(odd % 2 == 0){   //check if the number of odd elements is even; as the sum of even number of odd numbers will yield an even number
            cout << "0" << endl;
        }
        else{
            for(int i=0; i<n; i++) {
                if(arr[i] % 2 == 0){    //checking the number of operations needed to partisan from even to odd
                    int count = 0;
                    while(arr[i] % 2 == 0){
                        count++;
                        arr[i] /= 2;
                    ...
                    int count = 0;  //checking the number of operations needed to partisan from even to odd
                    while(arr[i] % 2 != 0){
                        count++;
                        arr[i] /= 2;
                    }
